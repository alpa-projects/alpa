...
from test_auto_sharding_basic import assert_close
...
def test_attention(mode='forward'):
    ...
    all_reduce_cost = 2 * (n_devices - 1) / n_devices * batch_size * seq_len * hidden_dim * 4
    assert_close(testing.last_compiled_auto_sharding_objective, all_reduce_cost)
...
def test_bert_layer(mode='forward'):
    ...
    n_devices = len(devices)
    all_reduce_cost = 2 * (n_devices - 1) / n_devices * batch_size * seq_len * hidden_dim * 4
    assert_close(testing.last_compiled_auto_sharding_objective, 3 * all_reduce_cost)
...
def test_n_bert_layer(mode='forward'):
    ...
    n_devices = len(devices)
    all_reduce_cost = 2 * (n_devices - 1) / n_devices * batch_size * seq_len * hidden_dim * 4
    assert_close(testing.last_compiled_auto_sharding_objective, 7 * all_reduce_cost)
