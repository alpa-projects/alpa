...
def mark_global_and_local_vars(stage: JaxPipelineStage, gensym_func):
    ...
    for eqn in stage.eqns:
        if eqn.primitive is pipeline_p and eqn.params['mark_type'] == 'start':
            ...
            outvars = [get_alias(var) for var in eqn.outvars + global_and_local_invars]
...
def generate_sharded_xla_stages(name: str, jax_stages: Sequence[JaxPipelineStage], stage_devices):
    invars = set()
    outvars = set()
    eqns = []
    consts_dir = {}
