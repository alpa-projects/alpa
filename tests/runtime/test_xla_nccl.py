...
from alpa.device_mesh import get_global_virtual_physical_mesh, ReshardingAllGatherSpec, next_tensor_uuids
...
class XLANCCLTest(unittest.TestCase):
    ...
    def test_xla_nccl_allgather(self):
        ...
        # Put buffers
        tensor_uuid = next_tensor_uuids(1)[0]
        ...
        for i in range(mesh.num_devices_per_host):
            ...
            shards.append(data)
        ray.get(worker.put_tensor.remote(tensor_uuid, shards, 1, 0))
        ...
        # Run allgather task
        ray.get(worker.run_allgather_task.remote(0, tensor_uuid))
        refs = ray.get(worker.get_buffers.remote(tensor_uuid))
        for i in range(4):
            for j in range(4):
