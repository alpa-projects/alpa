...
def get_train_step(model, tx, layer_num=8, labels_count=12):
    ...
    def train_step_alpa(params, opt_state, batch):
        ...
        loss, grads = alpa.value_and_grad(training_loss_fn)(params, batch)
...
def train_epoch(params, opt_state, train_loader, epoch, train_step):
    ...
    for batch in train_loader:
        ...
        params, opt_state, loss = train_step(params, opt_state, (jnp.array(batch[0]), jnp.array(batch[1])))
        end_time = time.time()
        losses.append(loss)
...
def create_train_state(rngkey, model, bsz, image_h, image_w, learning_rate_fn):
    dummy_input = {
        "x": jnp.zeros((bsz, image_h, image_w, 3), jnp.float32),
        "train": False
    }
...
def train_and_evaluate(config: ml_collections.ConfigDict,
    ...
    params, opt_state, tx = create_train_state(rng, model, config.batch_size, config.image_h, config.image_w, config.learning_rate)
    ...
    # params, opt_state = train_step_alpa(params, opt_state, (batch[0], batch[1]))
    batch = (jnp.zeros((config.batch_size, config.image_h, config.image_w, 3), 
                 dtype=np.float32), 
              jnp.ones((config.batch_size, config.image_h, config.image_w), dtype=np.int32))
    # params, opt_state, loss = train_step(params, opt_state, (jnp.array(batch[0]), jnp.array(batch[1])))
    # print(loss)
