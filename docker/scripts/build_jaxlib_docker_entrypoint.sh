...
case $2 in
  cuda)
    python build.py --enable_cuda --bazel_startup_options="--output_user_root=/build/root" --tf_path=$(pwd)/../third_party/tensorflow-alpa
    ;;
  nocuda)
    python build.py --enable_tpu --bazel_startup_options="--output_user_root=/build/root" --tf_path=$(pwd)/../third_party/tensorflow-alpa
    ;;
  *)
    usage
esac
