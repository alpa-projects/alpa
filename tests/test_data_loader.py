...
def input_iter_func(start, end, batch_size):
    ...
    for i in range(num_batches):
        yield (i * np.ones((batch_size, 32), dtype=np.float32),
...
class DataLoaderTest(unittest.TestCase):
    ...
    def run_test(self, sharding_specs):
        ...
        batch_size = 64
        num_samples = 256
        avals = [jax.core.ShapedArray((batch_size, 32), jnp.float32),
    ...
    def test_model_parallel(self):
        ...
        sharding_specs = [
            pxla.ShardingSpec(
                (pxla.NoSharding(), pxla.Chunked((num_devices,))),
                (pxla.ShardedAxis(0),)
            ),
            pxla.ShardingSpec(
                (pxla.NoSharding(),),
    ...
    def test_data_model_parallel(self):
        ...
        sharding_specs = [
            pxla.ShardingSpec(
                (pxla.Chunked((dp,)), pxla.Chunked((mp,))),
                (pxla.ShardedAxis(0), pxla.ShardedAxis(1))
            ),
            pxla.ShardingSpec(
                (pxla.Chunked((dp,)),),
                (pxla.ShardedAxis(0), pxla.Replicated(mp),)
            )
        ]
        self.run_test(sharding_specs)
