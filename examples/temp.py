...
set_parallelize_options(devices=devices, strategy="3d_parallel", pipeline_stage_mode="uniform_layer_gpipe")
...
def parallel_train_step(opt_state, params, batch):
    ...
    val, grads = alpa.value_and_grad(loss_func)(params, batch["x"], batch["y"])
    updates, opt_state = tx.update(grads, opt_state)
    params = optax.apply_updates(params, updates)
    return val, opt_state, params
...
def train_step(opt_state, params, batch):
    ...
    val, grads = jax.value_and_grad(loss_func)(params, batch["x"], batch["y"])
    updates, opt_state = tx.update(grads, opt_state)
    params = optax.apply_updates(params, updates)
    return val, opt_state, params
...
batch = {"x": x, "y": y}
loss_orig, state_orig, params_orig = train_step(opt_state, params, batch)
loss_with_pipeline, state_with_pipeline, params_with_pipeline = parallel_train_step(opt_state, params, batch)

# We check that the parameters are the same. 
print("Loss orig: ", loss_orig)
print("Loss with pipeline: ", loss_with_pipeline)
