...
class GpuHost:
    ...
    def benchmark_allreduce(self, size, dtype, groups):
        comm = None
        for group in groups:
        ...
        if self.global_id == 0:
            ...
            print(f"Group: {groups}\tBytes: {array_size / GB:.5f} GB\t"
                  f"Time: {time_cost:.5f} s\tBandwidth: {bandwidth / (1<<30):.2f} GB/s")

    def benchmark(self):
        for i in range(30):
            #self.benchmark_allreduce(1 << i, cp.float32, [[0, 1, 2, 3]])
            #self.benchmark_allreduce(1 << i, cp.float32, [[0, 4], [1, 5], [2, 6], [3, 7]])
            #self.benchmark_allreduce(1 << i, cp.float32, [[0, 2, 4, 6], [1, 3, 5, 7]])
            #self.benchmark_allreduce(1 << i, cp.float32, [[0, 1, 2, 3], [4, 5, 6, 7]])
            self.benchmark_allreduce(1 << i, cp.float32, [[0, 1, 2, 3, 4, 5, 6, 7]])
...
if __name__ == "__main__":
    ...
    ray.get([w.benchmark.remote() for w in workers])
