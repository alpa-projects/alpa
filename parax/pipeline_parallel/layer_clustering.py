...
def forward(fn : Callable, layer_num : int, eps : float = 0, use_remat : bool = False, remat_ratio : float = 1.0):
    ...
    def slice_by_solution(closed_jaxpr : ClosedJaxpr, sliced_eqns) -> List[ClosedJaxpr]:
      N = len(sliced_eqns)
      global_invars = set(closed_jaxpr.jaxpr.invars)
      ...
      for i, eqns in enumerate(sliced_eqns):
        for eqn in eqns:
          for var in eqn.invars:
            ...
            else: assert var_layer_dict[var] == i
          for var in eqn.outvars:
            if not isinstance(var, DropVar):
              var_layer_dict[var] = i
      ...
      for i, eqns in enumerate(sliced_eqns):
        new_jaxpr = Jaxpr(list(layer_consts[i].keys()), 
                          list(layer_invars[i]), 
    ...
    if use_remat: 
      ...
      def wrapped(*args):
        ...
        sliced_jaxprs = slice_by_solution(origin_jaxpr, solution)
        sliced_runnables = [jax.remat(jaxpr_as_fun(layer)) for layer in sliced_jaxprs]
