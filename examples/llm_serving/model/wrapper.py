...
def get_alpa_model(model_name: str,
    ...
    def inference_func(input_ids,
        ...
        global paddings
        if not paddings:
            # record padding and apply padding
            batch, seq_len = input_ids.shape
            paddings = [0] * batch
            for i in range(batch):
                for j in range(seq_len):
                    if input_ids.item((i,j)) == pad_token_id:
                        paddings[i] = j + 1
        # modify attention mask according to the padding
        for r in range(len(paddings)):
            for c in range(paddings[r]):
                attention_mask.itemset((r,c), 0)
