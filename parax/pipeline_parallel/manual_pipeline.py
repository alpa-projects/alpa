...
from parax.pipeline_parallel.stage import (slice_eqns_by_pipeline_marks, add_pipeline_marks_for_sliced_eqns)


def manual_pipeline(fn: Callable, static_argnums=()):
    @wraps(fn)
    def wrapped(*args):
        origin_jaxpr, out_shape_tree = make_jaxpr(fn, static_argnums=static_argnums, return_shape=True)(*args)
        sliced_eqns = slice_eqns_by_pipeline_marks(origin_jaxpr)
        new_jaxpr = add_pipeline_marks_for_sliced_eqns(origin_jaxpr, sliced_eqns)
